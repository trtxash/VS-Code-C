...
int main(void)
{
    ...
    */
    ...
    double b = 3; // 命名的第一个字符必须是字母或者下划线,例如合法的命名:name_123; name123_; _name123; _123name,不能以数字开头,不能包含除字母,下划线,数字以外的其他字符
    double c = 2;
    double delta;
    double X1;
    double X2;
    ...
    if (delta > 0) // 关系运算符： > < == != >= <=; -- 满足为1，不满足为0
    {
        X1 = (-b + sqrt(delta)) / (2 * a);
        X2 = (-b - sqrt(delta)) / (2 * a);
        printf("X1 = %lf,X2 = %lf\n", X1, X2); // printf("字符串")，printf("输出控制符"，输出参数)；printf("输出控制符1 输出控制符2"，输出参数1，输出参数2);输出参数运算为从右往左
        /*
